...
class DigraphWithWeights:
    ...
    def visualize(self):
        ...
        edge_labels = {(u, v): f"{weight}" for (u, v), weight in self.weights.items()}
    ...
    def zmien_wagi_na_ujemne(self):
        ...
        for edge in negative_edges:
            self.weights[edge] = -abs(self.weights[edge])
...
def bellman_ford(G, w, s):
    ...
    d = [float('inf')] * n
    helper = [None] * n
    d[s] = 0 #wierzchołek startowy
    
    for i in range(n - 1):
        for (u, v) in w:
            relax(d, u, v, w, helper)
    
    for (u, v) in w:
        if d[v] > d[u] + w[(u, v)]:
            return False #pojawił się cykl ujemny
...
def najkrotsz_sciezka(helper, node):
    path = []
    current = node
...
def main():    
    ...
    #przypisanie losowych wag, sum a wag w danym cyklu nie może być ujemna! (miejmy nadzieję ^^)
    graph_with_weights = dodaj_wagi(graph1.adjacency_list)
    graph_with_weights.zmien_wagi_na_ujemne()
    #print("\nDigraf z wagami:")
    #graph_with_weights.display()
    graph_with_weights.visualize()
    
    print("\n\n\n")
    for start_node in range(graph_with_weights.nodes):
        ...
        for node in range(graph_with_weights.nodes):
            if d[node] == float('inf'):
